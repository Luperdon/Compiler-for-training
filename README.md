...
Конечный автомат задаётся кортежем:
...
Разработанная грамматика относится к регулярному типу (тип 3) по классификации Хомского, так как все правила имеют вид A → aB или A → a. Это означает, что грамматика может быть распознана конечным автоматом, что и реализовано в синтаксическом анализаторе.

**Обоснование:**

1. Левая часть каждого правила содержит ровно один нетерминальный символ

   • Все правила имеют вид A → ..., где A — одиночный нетерминал

   • Пример: <IDREM> → Symbols <IDREM>

2. Правила имеют строго ограниченную форму (главный признак регулярной грамматики)

   - Праволинейные правила: A → aB (нетерминал → терминал + нетерминал)

   - Терминальные правила: A → a (нетерминал → терминал)

   - В грамматике НЕТ правил вида A → B a (леволинейных)

3. Отсутствуют правила с двумя нетерминалами подряд в правой части

   - Нет правил вида A → B C или A → a B C

   - Каждое правило содержит не более одного нетерминала, и он всегда находится в конце

4. Рекурсия только праволинейная (хвостовая)

   - Допустима только рекурсия вида A → a A

   - Пример: <INTREM> → digit <INTREM>

5. Применение правил не зависит от контекста

   - Правило можно применить всегда, независимо от окружающих символов

**Характеристики:**

| Характеристика | Описание |
|----------------|----------|
| Регулярная (тип 3) | Соответствует классу регулярных грамматик по Хомскому |
| Праволинейная | Нетерминал всегда находится в конце правой части правил |
| Детерминированная | Для каждой пары (состояние, входной символ) определён не более одного перехода |
| LL(1)-грамматика | Возможно построение LL(1)-анализатора (чтение слева направо, один символ вперед) |
| Не леворекурсивная | Отсутствуют правила вида `A → Aα` |
| Распознаётся конечным автоматом | Для синтаксического анализа достаточно конечного автомата без магазинной памяти |

**Примеры правил из грамматики:**

- `<INTREM> → digit <INTREM>` — праволинейное рекурсивное правило
- `<END> → ';'` — терминальное правило
- `<SCIENTIFIC> → '+' <INT>` — праволинейное правило
...
![Граф конечного автомата](screenshots/ГрафАвтоматаЛР3финал.png)
